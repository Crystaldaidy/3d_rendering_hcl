...
def projection(triangle_3d,triangle_2d,x):
    ...
    with hcl.if_(x==1):
        ...
        with hcl.else_():
            ...
            triangle_2d[x]=triangle_3d[1]
    
    #x1: x-y plane&x-z plane:x1; y-z plane:z1
    with hcl.if_(x==2):
        with hcl.if_(angle[0]==2):
            #return triangle_3d[5]
            triangle_2d[x]=triangle_3d[5]
        with hcl.else_():
            #return triangle_3d[3]
            triangle_2d[x]=triangle_3d[3]
    ...
    with hcl.if_(x==3):
        ...
        with hcl.else_():
            ...
            triangle_2d[x]=triangle_3d[4]
            
    #x2: x-y plane&x-z plane:x2; y-z plane:z2
    with hcl.if_(x==4):
        with hcl.if_(angle[0]==2):
            #return triangle_3d[8]
            triangle_2d[x]=triangle_3d[8]
        with hcl.else_():
            #return triangle_3d[6]
            triangle_2d[x]=triangle_3d[6]
    ...
    with hcl.if_(x==6):
        with hcl.if_(angle[0]==0):
            ...
            triangle_2d[x]=triangle_3d[2]/3+triangle_3d[5]/3+triangle_3d[8]/3
        with hcl.elif_(angle[0]==1):
            #return triangle_3d[1]/3+triangle_3d[4]/3+triangle_3d[7]/3
            triangle_2d[x]=triangle_3d[1]/3+triangle_3d[4]/3+triangle_3d[7]/3
        with hcl.elif_(angle[0]==2):
            #return triangle_3d[0]/3+triangle_3d[3]/3+triangle_3d[6]/3
            triangle_2d[x]=triangle_3d[0]/3+triangle_3d[3]/3+triangle_3d[6]/3
...
def rasterization(frag_cntr,triangle_2d,fragment):
    ...
    #   Determine whether three vertices of a trianlge (x0,y0) (x1,y1) (x2,y2) are in clockwise order by Pineda algorithm
    #   if so, return cw > 0
    ...
    with hcl.Stage("S1"):
        with hcl.for_(min_y,max_y) as y:
            with hcl.for_(min_x,max_x,name="x") as x:
...
def zculling(size_pixels,size,fragment,z_buffer,pixels):
    pixel_cntr = hcl.scalar(0)

    with hcl.Stage("S2"):
        with hcl.for_(0,size,name="n") as n:
...
def rendering(triangle_3ds,angle):
    ...
    def loop_body(m):
        triangle_3d = hcl.compute((9,),lambda x:triangle_3ds[m][x],"triangle_3d")
        ...
        hcl.mutate((1,),lambda x:rasterization(frag_cntr,triangle_2d,fragment),"fragment_update")
        
        # 3rd Stage Z-culling:update z_buffer,pixels
        hcl.mutate((1,),lambda x:zculling(size_pixels,frag_cntr[0],fragment,z_buffer,pixels),"z_update")
...
triangle_3ds = hcl.placeholder((num_3d_triangles,9),"triangle_3ds",hcl.Int())
angle = hcl.placeholder((1,),"angle",hcl.Int())
    
s = hcl.create_schedule([triangle_3ds,angle],rendering)

main_body = rendering.main_body
twod_update = main_body.twod_update
s_x = main_body.fragment_update.S1
s_n = main_body.z_update.S2
buffer_update = main_body.buffer_update

#s[twod_update].parallel(twod_update.axis[0])
s[main_body].pipeline(main_body.axis[0])
s[s_x].pipeline(s_x.x)
s[s_n].pipeline(s_n.n)
s[buffer_update].pipeline(buffer_update.axis[0])
#print(hcl.lower(s))

target = hcl.platform.zc706
s.to([triangle_3ds, angle], target.xcel)
s.to(main_body.frame_buffer,target.host)
...
start = time.time()
f(_triangle_3ds,_angle,_frame_buffer)
end = time.time()
